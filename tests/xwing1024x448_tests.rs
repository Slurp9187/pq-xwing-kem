//! Unit tests for xwing1024x448.

use pq_xwing_kem::xwing1024x448::{
    generate_keypair, Ciphertext, EncapsulationKey, XWING1024X448_CIPHERTEXT_SIZE,
    XWING1024X448_ENCAPSULATION_KEY_SIZE,
};
...
fn test_generate_keypair() {
    ...
    assert_eq!(pk.to_bytes().len(), XWING1024X448_ENCAPSULATION_KEY_SIZE);
}
...
fn test_encapsulation_decapsulation_roundtrip() {
    ...
    assert_eq!(ct.to_bytes().len(), XWING1024X448_CIPHERTEXT_SIZE);
}
...
fn test_wrong_key_decapsulate_fails() {
    ...
    // but X448 will give different ss_x, so overall different secret.
...
fn test_invalid_x448_public_key_validation() {
    ...
    let mut invalid_pk_bytes = [0u8; XWING1024X448_ENCAPSULATION_KEY_SIZE];
    // First 1568 bytes are ML-KEM key (leave as zeros for this test)
    // Last 56 bytes are X448 public key - set to all zeros (invalid)
    invalid_pk_bytes[XWING1024X448_ENCAPSULATION_KEY_SIZE - 56..].fill(0);

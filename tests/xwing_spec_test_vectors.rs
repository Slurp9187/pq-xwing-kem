...
use pq_xwing_kem::xwing768::{EncapsulationKey, generate_keypair};
use rand_core::OsRng;
...
#[allow(dead_code)] // Suppress warnings for unused fields (to be used in full implementation)
...
#[allow(dead_code)] // Suppress warning (to be used in full implementation)
...
fn test_official_xwing_test_vectors() {
    ...
    let test_vectors_json =
        fs::read_to_string("tests/test-vectors.json").expect("Failed to read test vectors file");
    let test_vectors: Vec<TestVector> =
        serde_json::from_str(&test_vectors_json).expect("Failed to parse test vectors JSON");
...
fn test_keypair_generation_sizes() {
    ...
    assert_eq!(
        enc_key.to_bytes().len(),
        1184 + 32,
        "Public key size incorrect"
    );
    ...
    assert_eq!(
        enc_key.pk_m().len(),
        1184,
        "ML-KEM public key size incorrect"
    );
    assert_eq!(
        enc_key.pk_x().to_bytes().len(),
        32,
        "X25519 public key size incorrect"
    );
}
